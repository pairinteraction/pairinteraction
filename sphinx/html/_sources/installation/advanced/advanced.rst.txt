...
For building from source we offer two different setups:
    :ref:`automatic` - This build method builds the C++ backend automatically, while allowing to build the python dependencies manually. Recommended for adjusting only the python code.

    :ref:`manual` - This build method allows to manually build both the C++ and the python backend. It allows to compile specific parts of the code only, define test targets, and others. Recommend for adjusting both the C++ and the python code.
...
You will find detailed instructions about how to set up your development environment here:
    :ref:`python_setup` - Get instructions on different python package management systems, and how to setup a virtual environment for development

    :ref:`system_setup` - Get instructions on the tooling chain to build the C++ backend via the :ref:`automatic <automatic>` or :ref:`manual <manual>` build.
