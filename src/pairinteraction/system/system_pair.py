...
class SystemPair(SystemBase[BasisPair]):
    ...
    def set_green_tensor(self, green_tensor: GreenTensorBase, interpolator_steps: int | None = None) -> Self:
        """Set the Green tensor for the pair system.

        Args:
            green_tensor: The Green tensor to set for the system.
            interpolator_steps: If the Green tensor is in the static limit, this argument must be None.
        ...
        if green_tensor.static_limit:
            if interpolator_steps is not None:
                raise ValueError(
                    "interpolator_steps must not be provided when using the static limit of the Green tensor."
                )
            gti = green_tensor.get_interpolator()
            self._set_green_tensor_interpolator(gti)
            return self

        if interpolator_steps is None:
            raise ValueError("interpolator_steps must be provided when not using the static limit of the Green tensor.")
        if interpolator_steps <= 0:
            raise ValueError("interpolator_steps must be a positive integer.")
        ...
        omegas = np.linspace(0, omega_max, interpolator_steps)
