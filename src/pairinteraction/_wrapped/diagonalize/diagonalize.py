...
def diagonalize(
    ...
    *,
    ...
    """
    if not all(e is None for e in energy_range):
        if not all(e is None for e in energy):
            raise ValueError("energy_range=... was replaced by energy=..., only use energy=...")
        warnings.warn(
            "The energy_range=... argument is deprecated, use energy=... instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        energy = energy_range
    ...
    for i, e in enumerate(energy):
        if e is not None:
            energy_range_au[i] = QuantityScalar.convert_user_to_au(e, energy_unit, "energy")
