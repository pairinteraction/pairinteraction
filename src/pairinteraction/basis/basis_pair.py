...
class BasisPair(BasisBase[KetPair, StatePair]):
    ...
    def get_corresponding_ket(self, state: StatePairLike) -> KetPair:  # type: ignore [override]
        # override the accepted state type
        return super().get_corresponding_ket(state)  # type: ignore [arg-type]

    def get_corresponding_ket_index(self, state: StatePairLike) -> int:  # type: ignore [override]
        # override the accepted state type
