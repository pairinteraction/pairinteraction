...
if __name__ == "__main__":
    ...
    for ax, efield in zip(axes, efields):
        ...
        for i1, i2 in zip(range(0, len(systems_two) - 1), range(1, len(systems_two))):
            ...
            segment_energies = (
                np.array(
                    [systems_two[i1].getHamiltonian().diagonal()[c1], systems_two[i2].getHamiltonian().diagonal()[c2]]
                )
                - fieldshift
            ) * 1e3  # MHz
            segment_overlap = np.mean(
                [systems_two[i1].getOverlap(state_two)[c1], systems_two[i2].getOverlap(state_two)[c2]], axis=0
            )
