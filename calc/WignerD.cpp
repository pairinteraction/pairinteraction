...
std::complex<double> WignerD::evalWignerdPiHalf(float j, float m, float mp) {
    ...
    r *= std::pow(-1., m-mp) / std::pow(2., j) * std::sqrt(boost::math::factorial<double>(j + m) * boost::math::factorial<double>(j - m) / (boost::math::factorial<double>(j + mp) * boost::math::factorial<double>(j - mp)));
    return r;
}
