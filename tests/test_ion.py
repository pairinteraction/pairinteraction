...
def test_ion() -> None:
    ...
    # Ensure that all eigenvalues are the same
    for system_x, system_y, system_z in zip(systems_x, systems_y, systems_z):
        np.testing.assert_allclose(system_x.get_eigenvalues(unit="GHz"), system_y.get_eigenvalues(unit="GHz"))
        np.testing.assert_allclose(system_x.get_eigenvalues(unit="GHz"), system_z.get_eigenvalues(unit="GHz"))
