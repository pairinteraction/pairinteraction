...
def test_static_green_tensor_pair_potential(pi_module: PairinteractionModule, distance_vector_mum: list[float]) -> None:
    if pi_module.__name__ == "pairinteraction.real" and distance_vector_mum[1] != 0:
        pytest.skip("The real backend does not support y-components in distance vectors.")
...
def rotation_matrix_from_axis_angle(axis: list[float], angle: float) -> NDArray:
    ...
    return np.eye(3) + np.sin(angle) * cross_matrix + (1 - np.cos(angle)) * cross_matrix @ cross_matrix
...
def test_surface_green_tensor_rotation_invariance() -> None:
    ...
    gt_rotated = GreenTensorSurface(
        ...
        normal=rotation.T @ np.array([0, 0, 1]),
        unit="micrometer",
        static_limit=True,
    ).get(1, 1, transition_energy=0, scaled=True)
...
def test_surface_green_tensor_rejects_zero_normal() -> None:
    with pytest.raises(ValueError, match="normal must not be the zero vector"):
        GreenTensorSurface(
            [0, 0, 0],
            [0, 0, 1],
