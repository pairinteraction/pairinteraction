...
def basis(pi_module: PairinteractionModule, system_atom: SystemAtom, system_atom2: SystemAtom) -> BasisPair:
    ...
    with no_log_propagation("pairinteraction.basis.basis_pair"):  # suppress number_of_kets warning
        return pi_module.BasisPair.from_kets(ket_atoms, system_atoms, number_of_kets=80)
...
def test_merge_pair_basis(pi_module: PairinteractionModule, system_atom: SystemAtom, system_atom2: SystemAtom) -> None:
    ...
    # merging is symmetric, i.e. the ket order does not depend on the argument order
    assert basis2.merge(basis1).kets == merged.kets
