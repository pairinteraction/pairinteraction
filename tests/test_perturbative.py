...
def test_c3_with_sample_system(system_pair_sample: pi.SystemPair) -> None:
    ...
    c3 = c3_obj.get_c3(unit="planck_constant * gigahertz * micrometer^3")
...
def test_c3() -> None:
    ...
    c3_obj.set_distance_vector([0, 0, 500], "micrometer")
...
def test_c6_with_sample_system(system_pair_sample: pi.SystemPair) -> None:
    ...
    c6 = c6_obj.get_c6(unit="planck_constant * gigahertz * micrometer^6")
...
def test_c6() -> None:
    ...
    c6_obj.set_distance_vector([0, 0, 500], "micrometer")

    c6 = c6_obj.get_c6(unit="planck_constant * gigahertz * micrometer^6")
    assert np.isclose(c6, 169.189)


def test_resonance_detection_inf(system_pair_sample: pi.SystemPair, caplog: pytest.LogCaptureFixture) -> None:
    """Test whether resonance with infinite admixture is correctly detected."""
    ket1 = pi.KetAtom("Rb", n=61, l=0, j=0.5, m=0.5)
    ket2 = pi.KetAtom("Rb", n=61, l=1, j=1.5, m=0.5)
    eff_h = perturbative.EffectiveHamiltonian([(ket1, ket2)])
...
def test_resonance_detection_resonance(caplog: pytest.LogCaptureFixture) -> None:
    """Test whether a resonance is correctly detected."""
