...
MatrixElementCache::CacheKey_cache_angular::CacheKey_cache_angular(int kappa, float j1, float j2, float m1, float m2)
    : kappa(kappa) {
    if ((j1 < j2) || ((j1 == j2) &&
                      (m1 <= m2))) {
        j = {{j1,j2}};
...
MatrixElementCache::CacheKey_cache_reduced_commutes::CacheKey_cache_reduced_commutes(float s, int kappa, int l1, int l2, float j1, float j2)
    : s(s), kappa(kappa) {
    if ((l1 < l2) || ((l1 == l2) &&
                      (j1 <= j2))) {
        l = {{l1,l2}};
...
MatrixElementCache::CacheKey_cache_reduced_multipole::CacheKey_cache_reduced_multipole(int kappa, int l1, int l2)
    : kappa(kappa) {
    if (l1 <= l2) {
        l = {{l1,l2}};
        sgn = 1;
...
bool MatrixElementCache::CacheKey_cache_angular::operator==(const CacheKey_cache_angular &rhs) const {
    return (kappa == rhs.kappa) && (j == rhs.j) && (m == rhs.m);
}

bool MatrixElementCache::CacheKey_cache_reduced_commutes::operator==(const CacheKey_cache_reduced_commutes &rhs) const {
    return (s == rhs.s) && (kappa == rhs.kappa) && (l == rhs.l) && (j == rhs.j);
}

bool MatrixElementCache::CacheKey_cache_reduced_multipole::operator==(const CacheKey_cache_reduced_multipole &rhs) const {
...
std::size_t MatrixElementCache::CacheKeyHasher_cache_radial::operator()(const CacheKey_cache_radial &c) const {
    size_t seed = 0;
...
std::size_t MatrixElementCache::CacheKeyHasher_cache_angular::operator()(const CacheKey_cache_angular &c) const {
    size_t seed = 0;
    boost::hash_combine(seed, c.kappa);
    boost::hash_combine(seed, c.j);
...
std::size_t MatrixElementCache::CacheKeyHasher_cache_reduced_commutes::operator()(const CacheKey_cache_reduced_commutes &c) const {
    size_t seed = 0;
...
std::size_t MatrixElementCache::CacheKeyHasher_cache_reduced_multipole::operator()(const CacheKey_cache_reduced_multipole &c) const {
    size_t seed = 0;
    boost::hash_combine(seed, c.kappa);
    boost::hash_combine(seed, c.l);
    return seed;
...
// TODO improve methods for precalculation, for every method getSomething create the method precalculateSomething
...
void MatrixElementCache::precalculateElectricMomentum(const std::vector<StateOne> &basis_one, int q) {
    precalculate(basis_one, 1, q, 1, true, false, false);
}

void MatrixElementCache::precalculateMagneticMomentum(const std::vector<StateOne> &basis_one, int q) {
    precalculate(basis_one, 1, q, 0, false, true, false);
}

void MatrixElementCache::precalculateDiamagnetism(const std::vector<StateOne> &basis_one, int k, int q) {
        ...
        for (size_t idx_row = 0; idx_row <= idx_col; ++idx_row) {
            ...
            if ((calcElectricMultipole && selectionRulesMultipoleNew(state_row, state_col, kappa_angular)) || (calcMagneticMomentum && selectionRulesMomentumNew(state_row, state_col)) || calcRadial) {
                if (calcElectricMultipole || calcMagneticMomentum || calcRadial) {
                    auto key = CacheKey_cache_radial(method, species, kappa_radial, state_row.n, state_col.n, state_row.l, state_col.l, state_row.j, state_col.j);
                ...
                if (calcElectricMultipole || calcMagneticMomentum) {
                    auto key = CacheKey_cache_angular(kappa_angular, state_row.j, state_col.j, state_row.m, state_col.m);
                ...
                if (calcElectricMultipole || calcMagneticMomentum) {
                    auto key = CacheKey_cache_reduced_commutes(s, kappa_angular, state_row.l, state_col.l, state_row.j, state_col.j);
                ...
                if (calcElectricMultipole) {
                    auto key = CacheKey_cache_reduced_multipole(kappa_angular, state_row.l, state_col.l);
...
int MatrixElementCache::update() {
    ...
    if (!dbname.empty()) {
        ...
        if (cache_radial_missing.size() > 0) {
            ...
            for (auto cached = cache_radial_missing.begin(); cached != cache_radial_missing.end();) {
        ...
        if (cache_angular_missing.size() > 0) {
            ...
            for (auto cached = cache_angular_missing.begin(); cached != cache_angular_missing.end();) {
        ...
        if (cache_reduced_commutes_s_missing.size() > 0) {
            ...
            for (auto cached = cache_reduced_commutes_s_missing.begin(); cached != cache_reduced_commutes_s_missing.end();) {
        ...
        if (cache_reduced_commutes_l_missing.size() > 0) {
            ...
            for (auto cached = cache_reduced_commutes_l_missing.begin(); cached != cache_reduced_commutes_l_missing.end();) {
                stmt.bind(1, cached->s);
                stmt.bind(2, cached->kappa);
                stmt.bind(3, cached->l[0]);
                stmt.bind(4, cached->j[0]);
        ...
        if (cache_reduced_multipole_missing.size() > 0) {
            ...
            for (auto cached = cache_reduced_multipole_missing.begin(); cached != cache_reduced_multipole_missing.end();) {
                stmt.bind(1, cached->kappa);
    ...
    if (cache_reduced_commutes_s_missing.size() > 0) {
        ...
        for (auto &cached : cache_reduced_commutes_s_missing) {
            ...
            double val = 0;
            if (cached.l[0] >= std::abs(cached.j[0]-cached.s) && cached.l[0] <= cached.j[0]+cached.s &&
                    cached.l[0] >= std::abs(cached.l[1]-cached.kappa) && cached.l[0] <= cached.l[1]+cached.kappa &&
                    cached.j[1] >= std::abs(cached.j[0]-cached.kappa) && cached.j[1] <= cached.j[0]+cached.kappa &&
                    cached.j[1] >= std::abs(cached.l[1]-cached.s) && cached.j[1] <= cached.l[1]+cached.s) { // TODO implement this in the wignerSymbols library
                val = pow(-1,int(cached.l[0] + cached.s + cached.j[1] + cached.kappa)) * sqrt((2*cached.j[0]+1)*(2*cached.j[1]+1)) *
    ...
    if (cache_reduced_commutes_l_missing.size() > 0) {
        ...
        for (auto &cached : cache_reduced_commutes_l_missing) {
            ...
            // Check triangle conditions (violated e.g. for WignerSymbols::wigner6j(0, 0.5, 0.5, 0.5, 0, 1))
            double val = 0;
            if (cached.s >= std::abs(cached.j[0]-cached.l[0]) && cached.s <= cached.j[0]+cached.l[0] &&
                    cached.s >= std::abs(cached.s-cached.kappa) && cached.s <= cached.s+cached.kappa &&
                    cached.j[1] >= std::abs(cached.j[0]-cached.kappa) && cached.j[1] <= cached.j[0]+cached.kappa &&
                    cached.j[1] >= std::abs(cached.s-cached.l[0]) && cached.j[1] <= cached.s+cached.l[0]) { // TODO implement this in the wignerSymbols library
