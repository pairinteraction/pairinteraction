...
double V(QuantumDefect const &qd, double x)
{
    ...
    double V_c = -Z_l / x;
    double V_p =
        -qd.ac / (2. * x * x * x * x) * (1 - std::exp(-std::pow(x / qd.rc, 6)));
    double V_so = 0.0;
    if (qd.l < 4) {
        double s = 0.5;
        ...
        double alpha =
            7.2973525664e-3; // ~1/137 fine-structure constant CODATA 2014
        V_so = alpha * alpha / (4 * x * x * x) *
               (qd.j * (qd.j + 1) - qd.l * (qd.l + 1) - s * (s + 1));
    }
    return V_c + V_p + V_so;
}
...
{
    return (2. * qd.l + .5) * (2. * qd.l + 1.5) / x +
           8 * x * (V(qd, x) - qd.energy / au2GHz);
}
...
Numerov::Numerov(QuantumDefect const &qd) : qd(qd)
{
    ...
    double xmin =
        qd.n * qd.n - qd.n * std::sqrt(qd.n * qd.n - (qd.l - 1) * (qd.l - 1));
    if (xmin < 2.08) {
...
eigen_dense_double_t Numerov::integrate()
{
    ...
    for (int i = nsteps - 3; i >= 0; --i) {
        double A =
            (2. + 5. / 6. * dx * dx * g(qd, xy(i + 1, 0) * xy(i + 1, 0))) *
            xy(i + 1, 1);
        double B =
            (1. - 1. / 12. * dx * dx * g(qd, xy(i + 2, 0) * xy(i + 2, 0))) *
            xy(i + 2, 1);
...
{
    return std::exp(-.5 * z) * std::pow(z, m + .5) *
           HypergeometricU(m - k + .5, 1 + 2 * m, z);
}

double RadialWFWhittaker(double r, double nu, int l)
{
    return 1 /
           std::sqrt(nu * nu * std::tgamma(nu + l + 1) * std::tgamma(nu - l)) *
