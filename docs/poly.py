...
# logging.basicConfig(level=logging.DEBUG, format="%(asctime)s - %(levelname)s - %(message)s")  # noqa: ERA001
...
for t in sorted([t.name for t in _all_tags], reverse=True):
    if any(w.startswith(t[:4]) for w in WANTED_TAGS):
        continue
